...
- `helpers.py`: `clean_json_response()` repairs LLM output (strips markdown, fixes quotes, removes trailing commas); `sanitize_text()` uses bleach; `get_rag_tools_path()` returns path to RAG tools metadata
